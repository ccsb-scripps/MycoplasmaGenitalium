...
C
C Relaxes lattice model and adds spheres for proteins
C
C Reads from unit 5:
C MG149_lattice.1.pdb  # input lattice coordinate files
C MG149.const          # input constraint file
C MG149_relax.1.pdb    # output coordinate files
C 80.                  # weight for repulsion
C 1,42.6,42.6          # flag to constraint cell size, radius and half length (lattice units)
C 3000,1234            # steps for optimization, random seed
C
...
C Proteins: PRO, with types specified in the atom name, such as GYR and RIB
C
...
C	calculate simple mutual repulsion vector
	...
	do izb=-5,5
	 ...
	 iy=y(i)+iyb
	 iz=z(i)+izb
	 rweight=float(ixb*ixb+iyb*iyb+izb*izb)
	 rweight=max(rweight,1.)
	 if ((nspace(ix,iy,iz).ne.0).and.(rweight.le.25.)) then
	 do ic=1,nspace(ix,iy,iz)
	   icatom=cspace(ix,iy,iz,ic)
	   if (abs(icatom-i).gt.8) then
	     rx=rx+(x(icatom)-x(i))/rweight
	     ry=ry+(y(icatom)-y(i))/rweight
	     rz=rz+(z(icatom)-z(i))/rweight
	   endif
	enddo
	endif
	if (rx.ne.0) rxsum=rxsum+rx
	if (ry.ne.0) rysum=rysum+ry
	if (rz.ne.0) rzsum=rzsum+rz

	enddo
	enddo
	enddo

C this is the weight for repulsion...
...
C --end constrain to cell sphere
...
C models for RNA
	read(pdb(i)(23:26),*) irna
	if (((pdb(i)(18:20).eq."MET").or.
     &       (pdb(i)(18:20).eq."CYS")).and.
     &       (irnalast.gt.irna)) then
	  write(1,12) "ENDMDL"
	  imodel=imodel+1
	  write(1,12) "MODEL ", imodel
	endif
